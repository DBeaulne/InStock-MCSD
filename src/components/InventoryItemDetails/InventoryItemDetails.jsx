...
export default function InventoryItemDetails() {
  ...
  const getWarehouseName = async (warehouse_id) => {
    try {
      const response = await axios.get(`${apiUrl}/warehouses/${warehouse_id}`);
      setWarehouseName(response.data.warehouse_name);
    } catch (error) {
      console.log(`Error while attempting to get warehouse name: ${error}`);
    }
  };
      ...
      <div className="item-details__wrapper">
        <div className="item-details__left-bin">
          <div className="item-details__info">
            <h3 className="item-details__subheading">ITEM DESCRIPTION:</h3>
        ...
        <div className="item-details__right-bin">
          <div className="right-bin__top-container">
            <div className="item-details__info right-bin__element">
              <h3 className="item-details__subheading">STATUS:</h3>

              <p
                className={`item-details__text ${
                  status === "In Stock" ? "in-stock--true" : "in-stock--false"
                }`}
              >
                {status}
              </p>
            ...
            <div className="item-details__info right-bin__element">
              <h3 className="item-details__subheading">QUANTITY:</h3>
          ...
          <div className="right-bin__bottom">
            <div className="item-details__info">
              <h3 className="item-details__subheading">WAREHOUSE:</h3>

              <p className="item-details__text">{warehouseName}</p>
            </div>
          </div>
        </div>
      </div>
